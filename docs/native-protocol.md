...
The stored PID is lifecycle metadata, not process authority. `close_container` succeeds only when the current native-host process still holds the in-memory managed process created for that launch. It never reopens or kills a process from a persisted PID.
